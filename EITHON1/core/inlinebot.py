...
def main_menu():
    text = f"**- [سـورس ايــثــون](https://t.me/EITHON1)\
        \n المسـاعد\
        \n\
        \n◽ ايــثــون لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("EITHON1"):
            buttons = [
                (
                    Button.inline("الحـالة ⚒️", data="stats"),
                    Button.url("Ξ𝗜𝗧𝗛𝗢𝗡™ UsᴇʀBoᴛ", "https://t.me/EITHON1"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./EITHON1", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس ايــثــون 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./EITHON1", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس ايــثــون 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./EITHON1", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس ايــثــون 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© EITHON1 Help",
                description="Help menu for EITHON1",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/EITHON1"),
                Button.url(
                    "كـروب المـساعدة 📬",
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت ايــثــون 🧸♥](https://t.me/EITHON1)**\
            \n\
            \n❤ ايـــثــون هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/eithonsupport)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
...
@EITHON1.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@EITHON1.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@EITHON1.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@EITHON1.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
