    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            USER_BOT_WARN_ZERO = custompmblock.format(
                ...
                warns=warns,
                remwarns=remwarns,
            )#تم تعديل اللهجه الى السوريه بواسطه مطورين ايــثــون
        else:
            USER_BOT_WARN_ZERO = f"⌯︙حذࢪتك وَِٰقݪتڪ لا تكࢪࢪ تَم حظࢪك بنجاح ما أإقدࢪ اخليك تزعج المالك \n- ⌯︙بباي 🙁🤍"
...
حاليا ماتقدر بعد تتكلم الى ان ياتي صاحب الحساب ويقوم بالغاء الحظر. 🙂💘"
...
**اتمنى ما تكرر الرسائل ماتجبرني احظرك 🙂🌿**"""
        await event.reply(text)
        PM_WARNS[str(chat.id)] = 1
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
@EITHON1.ar_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
#
@EITHON1.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
#
@EITHON1.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@EITHON1.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⌯︙- عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك 🧸♥"
        return await event.answer(text, cache_time=0, alert=True)
...
#
@EITHON1.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⌯︙- عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك 🧸♥"
        return await event.answer(text, cache_time=0, alert=True)
...
@EITHON1.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = " عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك 🧸♥"
        return await event.answer(text, cache_time=0, alert=True)
    ...
    try:
        ...
        PM_WARNS = {}#تم تعديل هذا الملف الى اللهجه السوريه بواسطه مطورين ايــثــون
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
async def pmpermit_on(event):
    ...
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None: #تم تعديل هذا الملف الى اللهجه السوريا بواسطه مطورين ايــثــون
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائـمة الحـماية ايــثــون.txt",
