#    جميع الحقوق محفوظة كتابة وتعديل  :   @TTTLL0
#    
marculs=9
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @EITHON1", str(err))           
...
async def gben(EITHON1):
    ...
    me = await EITHON1.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await EITHON1.get_chat()
    if EITHON1.is_private:
        user = EITHON1.chat
        rank = EITHON1.pattern_match.group(1)
    else:
        EITHON1.chat.title
    try:
        user, rank = await get_full_user(EITHON1)
    except:
        pass
    if me == user:
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await EITHON1.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await EITHON1.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
