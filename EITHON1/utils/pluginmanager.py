...
from EITHON1 import CMD_HELP, LOAD_PLUG
...
LOGS = logging.getLogger("ايــثــون")
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        checkplugins(path)
        name = "EITHON1.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"EITHON1/plugins/{shortname}.py")
            name = f"EITHON1.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["EITHON1.plugins." + shortname] = mod
        LOGS.info("⌯︙تم بنجاح تحميل ملف " + shortname)
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"EITHON1/plugins/assistant/{shortname}.py")
        name = "EITHON1.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["EITHON1.plugins.assistant" + shortname] = mod
